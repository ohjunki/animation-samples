...
class MainActivity : AppCompatActivity(), CompoundButton.OnCheckedChangeListener {
    ...
    private val dataset: Array<DemosAdapter.Demo> = arrayOf(
        ...
        DemosAdapter.Demo("ImageFilterView", "Show image cross-fade (using ML's ImageFilterView + custom attribute)", R.layout.motion_04_imagefilter),
        DemosAdapter.Demo("Complex MotionLayout", "Advanced Synchronized reval motion + helper (bounce). Implemented with MotionLayout only.", R.layout.motion_20_reveal),
        DemosAdapter.Demo("AppBarLayout with MotionLayout", "Another AppBarLayout replacement example", R.layout.motion_11_coordinatorlayout),
        DemosAdapter.Demo("ViewPager Lottie Example", "Using MotionLayout and Lottie with ViewPager", ViewPagerActivity2::class.java),
        DemosAdapter.Demo("Imple CoordinatorLayout with Only Motion", "Advanced CoordinatorLayout-like behavior (adding a FAB). Implemented with MotionLayout only, using direct resizing of the view.", R.layout.motion_19_coordination),
